    ...
    <div className="flex items-start gap-4 py-4 border-b border-gray-100 last:border-b-0">
        ...
        <div className="flex-1">
            <div className="flex items-center gap-3">
                <p className="font-bold text-primary-text">{user?.name || 'Anonymous'}</p>
                <p className="text-xs text-secondary-text">
                    {new Date(date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                </p>
            </div>
            <p className="mt-1 text-gray-700 leading-relaxed whitespace-pre-wrap">{text}</p>
        </div>
    </div>
);
...
const CommentSection = ({ recipeId, initialCommentCount = 0 }) => {
    ...
    useEffect(() => {
        ...
        const fetchComments = async () => {
            setLoading(true);
            try {
                const { data } = await API.get(`/api/comments/recipe/${recipeId}`, { signal });
    ...
    }, [newComment, recipeId]); // Note dependencies
        ...
        <div id="comment-section" className="mt-16 pt-12 border-t border-gray-200">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm mb-12">
                {userInfo ? (
                    <form onSubmit={handleCommentSubmit} className="space-y-4">
                        <div>
                            <label htmlFor="comment-textarea" className="font-semibold text-gray-700 sr-only">Your Comment</label>
                            <textarea
                                id="comment-textarea"
                                value={newComment}
                        ...
                        <div className="flex justify-end">
                            <button type="submit" disabled={isSubmitting || !newComment.trim()} className="bg-accent text-white font-semibold py-2 px-6 rounded-md hover:bg-opacity-90 transition-colors disabled:bg-accent/50 disabled:cursor-not-allowed">
                    ...
                    <div className="text-center p-4 bg-gray-50/70 rounded-md">
                        <p className="text-secondary-text">
                            <Link to="/login" className="text-accent font-bold hover:underline">Log in</Link> or <Link to="/signup" className="text-accent font-bold hover:underline">sign up</Link> to join the discussion!
                        </p>
                    </div>
                )}
            </div>
            ...
            <div className="space-y-2">
                {loading ? (
                    ...
                    <p className="text-secondary-text text-center py-8">Be the first to share your thoughts!</p>
                )}
            </div>
        </div>
    );
